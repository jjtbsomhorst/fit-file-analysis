...
/**
 * Demonstration of the FITFileAnalysis class using Twitter Bootstrap framework
...
try {
  ...
  $pFFA = new TrainerPlan\FIT\FITFileAnalysis(__DIR__ . $file, $options);
...
<head>
  <meta charset="utf-8">
  <title>FITFileAnalysis demo</title>
...
<body>
  <div class="jumbotron">
    <div class="container">
      <h2><strong>FITFileAnalysis </strong><small>A PHP class for analysing FIT files created by Garmin GPS devices.</small></h2>
      <p>This is a demonstration of the FITFileAnalysis class available on <a class="btn btn-default btn-lg" href="https://github.com/adriangibbons/phpFITFileAnalysis" target="_blank" role="button"><i class="fa fa-github"></i> GitHub</a></p>
    </div>
  </div>
  <div class="container">
    <div class="col-md-6">
  ...
  <script type="text/javascript">
    $(function() {
      ...
      tsDim = ndx.dimension(function(d) {
        return d.timestamp;
      });

      latlngDim = ndx.dimension(function(d) {
        return [d.position_lat, d.position_long];
      });

      qaDim = ndx.dimension(function(d) {
        return [d.cpv, d.aepf, d.lap];
      });
      qaGrp = qaDim.group().reduceSum(function(d) {
        return d.lap;
      });

      lapDim = ndx.dimension(function(d) {
        return d.lap;
      });
      lapGrp = lapDim.group();

      cadDim = ndx.dimension(function(d) {
        return d.cadence;
      });
      cadGrp = cadDim.group();

      hrDim = ndx.dimension(function(d) {
        return d.heart_rate;
      });
      hrGrp = hrDim.group();
      ...
      qaScatterChart.on('renderlet', function(chart) {
        var horizontal_line = [{
            x: chart.x().range()[0],
            y: chart.y()(ride_data.aepf_threshold)
          },
          {
            x: chart.x().range()[1],
            y: chart.y()(ride_data.aepf_threshold)
        ...
        var vertical_line = [{
            x: chart.x()(ride_data.cpv_threshold),
            y: chart.y().range()[0]
          },
          {
            x: chart.x()(ride_data.cpv_threshold),
